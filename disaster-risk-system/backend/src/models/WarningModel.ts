...
export interface Warning {
  ...
  shelter_recommendations?: any; // JSONB
  status: string;
...
export class WarningModel {
  ...
  static async findById(id: number): Promise<Warning | null> {
    const query = `
      SELECT w.*, dt.name as disaster_type_name, rz.name as zone_name
      FROM warnings w
      LEFT JOIN disaster_types dt ON w.disaster_type_id = dt.id
  ...
  static async findByWarningId(warningId: string): Promise<Warning | null> {
    const query = `
      SELECT w.*, dt.name as disaster_type_name, rz.name as zone_name
      FROM warnings w
      LEFT JOIN disaster_types dt ON w.disaster_type_id = dt.id
    ...
    `;
    const result = await pool.query(query, [warningId]);
    return result.rows[0] || null;
  }

  static async create(data: CreateWarningData): Promise<Warning> {
    const query = `
  ...
  static async update(id: number, data: UpdateWarningData): Promise<Warning | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;
  ...
  static async findByZone(zoneId: number): Promise<Warning[]> {
    const query = `
      SELECT w.*, dt.name as disaster_type_name, rz.name as zone_name,
             ST_AsText(w.affected_area) as affected_area_wkt
      FROM warnings w
  ...
  static async warningIdExists(warningId: string): Promise<boolean> {
    const query = 'SELECT id FROM warnings WHERE warning_id = $1';
    const result = await pool.query(query, [warningId]);
    return result.rows.length > 0;
  }

  static async markExpiredWarnings(): Promise<number> {
    const query = `
      UPDATE warnings 
      SET status = 'expired' 
  ...
  static async getWarningStats(): Promise<any> {
    const query = `
      SELECT 
