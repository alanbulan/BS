...
export class ShelterModel extends BaseModel {
  constructor() {
    super('shelters');
  }

  // 创建避难场所
  async create(data: Omit<Shelter, 'id' | 'created_at' | 'updated_at'>): Promise<Shelter> {
    const sql = `
  ...
  }): Promise<{ data: Shelter[]; pagination: any }> {
  ...
  // 获取避难场所类型列表
  async getShelterTypes(): Promise<string[]> {
    const sql = `
  ...
  // 更新避难场所占用情况
  async updateOccupancy(id: number, occupancy: number): Promise<Shelter | null> {
    const sql = `
      UPDATE shelters 
  ...
  // 获取容量利用率高的避难场所
  async getHighOccupancyShelters(threshold: number = 0.8): Promise<Shelter[]> {
    const sql = `
      SELECT *,
