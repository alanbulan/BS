...
import { UserReport, Point, LocationQuery } from '../types';
...
export class UserReportModel extends BaseModel {
  ...
  /**
   * 重写findById方法，联查users表返回用户信息
   */
  async findById(id: number): Promise<UserReport | null> {
    const sql = `
      SELECT 
        ur.*,
   ...
   * 创建用户报告
   */
  async create(data: Omit<UserReport, 'id' | 'created_at' | 'updated_at'>): Promise<UserReport> {
    const sql = `
  ...
  async findWithPagination(options: {
  ...
  }): Promise<{ data: UserReport[]; pagination: any }> {
    ...
    if (options.conditions) {
      const { 
        user_id, report_type, disaster_type_id, verification_status, 
        min_severity, max_severity, is_emergency, start_date, end_date,
        search
      } = options.conditions;
   ...
   * 根据位置查找附近的用户报告
   */
  async findNearby(location: LocationQuery, options: {
    report_type?: string;
   ...
   * 更新投票数
   */
  async updateVotes(id: number, voteType: 'upvote' | 'downvote'): Promise<UserReport | null> {
    const sql = `
      UPDATE user_reports
   ...
   * 获取报告类型统计
   */
  async getReportTypeStats(): Promise<any[]> {
    const sql = `
      SELECT 
        report_type,
        COUNT(*) as count,
